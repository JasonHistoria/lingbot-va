...
try:
    ...
    try:
        ...
        # flash-attn is only used when attn_mode='flashattn' — inference with
        # attn_mode='torch' (SDPA) doesn't need it. The error is deferred to
        # the WanAttention __init__ branch below so an env without flash-attn
        # remains usable for the SDPA path.
    ...
    def __init__(
        ...
        if attn_mode == 'torch':
            ...
            if flash_attn_func is None:
                raise ImportError(
                    "attn_mode='flashattn' requires flash-attn to be installed "
                    "(`pip install flash-attn --no-build-isolation`). Use "
                    "attn_mode='torch' for SDPA fallback."
                )
